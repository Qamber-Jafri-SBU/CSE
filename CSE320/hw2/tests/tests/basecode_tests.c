...
/*
 * Start the program, execute a refresh command, then quit.
 */
Test(base_suite, start_refresh_test, .timeout=30) {
    char *name = "start_refresh";
    sprintf(program_options, "tests/rsrc/test_dir");
    int err = run_using_system(name, name, "", "", STANDARD_LIMITS);
    assert_normal_exit(err);
    assert_outfile_matches(name, NULL);
...
Test(base_suite, valgrind_leak_test, .init=setup_test_dir, .init=setup_test_dir) {
    ...
    sprintf(program_options, "tests/rsrc/test_dir");
    int err = run_using_system(name, name, "", "valgrind --leak-check=full --undef-value-errors=no --error-exitcode=37", STANDARD_LIMITS);
...
Test(base_suite, valgrind_undef_test, .init=setup_test_dir, .init=setup_test_dir) {
    ...
    sprintf(program_options, "tests/rsrc/test_dir");
    int err = run_using_system(name, name, "", "valgrind --undef-value-errors=yes --error-exitcode=37", STANDARD_LIMITS);
    assert_no_valgrind_errors(err);
    assert_normal_exit(err);
    assert_outfile_matches(name, NULL);
}
