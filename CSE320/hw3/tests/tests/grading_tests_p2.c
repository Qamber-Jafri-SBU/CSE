...
Test(sf_memsuite_grading, malloc_free_lifo, .timeout=TEST_TIMEOUT)
{
    ...
    _assert_block_info((sf_block *)((char *)u - 16), 1, 256);
    void * y = sf_malloc(sz);
    _assert_nonnull_payload_pointer(y);
    _assert_block_info((sf_block *)((char *)y - 16), 1, 256);
    void * v = sf_malloc(sz);
    _assert_nonnull_payload_pointer(v);
    _assert_block_info((sf_block *)((char *)v - 16), 1, 256);
    void * z = sf_malloc(sz);
    _assert_nonnull_payload_pointer(z);
    _assert_block_info((sf_block *)((char *)z - 16), 1, 256);
    void * w = sf_malloc(sz);
    _assert_nonnull_payload_pointer(w);
    _assert_block_info((sf_block *)((char *)w - 16), 1, 256);
    ...
    _assert_block_info((sf_block *)((char *)z1 - 16), 1, 256);
    void * y1 = sf_malloc(sz);
    _assert_nonnull_payload_pointer(y1);
    _assert_block_info((sf_block *)((char *)y1 - 16), 1, 256);
    void * x1 = sf_malloc(sz);
    _assert_nonnull_payload_pointer(x1);
    _assert_block_info((sf_block *)((char *)x1 - 16), 1, 256);
...
Test(sf_memsuite_grading, realloc_larger, .timeout=TEST_TIMEOUT)
{
    ...
    _assert_nonnull_payload_pointer(y);
    _assert_block_info((sf_block *)((char *)y - 16), 1, 1088);
    _assert_free_block_count(256, 1);
    _assert_free_block_count(6720, 1);
...
Test(sf_memsuite_grading, realloc_smaller, .timeout=TEST_TIMEOUT)
{
    ...
    void * x = sf_malloc(sz);
    _assert_nonnull_payload_pointer(x);
    _assert_block_info((sf_block *)((char *)x - 16), 1, 1088);
...
Test(sf_memsuite_grading, realloc_same, .timeout=TEST_TIMEOUT)
{
    ...
    _assert_free_block_count(6976, 1);
...
Test(sf_memsuite_grading, realloc_size_0, .timeout=TEST_TIMEOUT)
{
    ...
    _assert_block_info((sf_block *)((char *)x - 16), 0, 1088);

    // after realloc x to (2) z, x is now a free block
    _assert_free_block_count(1088, 1);

    // the size of the remaining free block
    _assert_free_block_count(5888, 1);
...
Test(sf_memsuite_grading, free_prev_alloc, .signal = SIGABRT, .timeout = TEST_TIMEOUT)
{
    ...
    void * w = sf_malloc(sz);
    void * x = sf_malloc(sz);
	*((char *)x - 8) &= ~PREV_BLOCK_ALLOCATED;
    sf_free(x);
    sf_free(w);
    cr_assert_fail("SIGABRT should have been received");
}

Test(sf_memsuite_grading, free_prologue, .signal = SIGABRT, .timeout = TEST_TIMEOUT)
{
    size_t sz = 1;
    void * x = sf_malloc(sz);
    sf_free(x - 64);
